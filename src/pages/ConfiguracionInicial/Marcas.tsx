...
const URL = 'https://oracleapex.com/ords/lasenda/marcas/listar';
const CREAR_URL = 'https://oracleapex.com/ords/lasenda/marcas/crear';
const ACTUALIZAR_URL = 'https://oracleapex.com/ords/lasenda/marcas/actualizar';
const ELIMINAR_URL = 'https://oracleapex.com/ords/lasenda/marcas/eliminar';
...
export function Marcas() {
  ...
  const [sel, setSel] = useState<Marca | null>(null);
  const [form, setForm] = useState<{ mode: 'crear' | 'editar'; marca: Marca | null } | null>(null);
  const [del, setDel] = useState<Marca | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    const list = t
      ? items.filter((m) => m.descripcion.toLowerCase().includes(t) || String(m.id_marca).includes(t))
      : items;
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      <Header title="Marcas" subtitle="Configuración Inicial" onAction={() => setForm({ mode: 'crear', marca: null })} />
      ...
      {loading ? (
        ...
        <EmptyState icon="ribbon-outline" message={q ? 'Sin resultados para tu búsqueda.' : 'No hay marcas cargadas.'} />
      ) : (
        <>
          <div className="art-count">{filtered.length} marca{filtered.length !== 1 ? 's' : ''}</div>
          <div className="art-list">
            {filtered.map((m) => (
              <button key={m.id_marca} className="art-card" onClick={() => setSel(m)}>
                <div className="art-info">
                  <div className="art-name">{m.descripcion.trim()}</div>
                  <div className="art-meta">
                    <span className="art-chip">#{m.id_marca}</span>
                  </div>
                </div>
      ...
      {form && (
        <MarcaForm
          ...
          onSave={async (descripcion) => {
            ...
            const url = crear ? CREAR_URL : `${ACTUALIZAR_URL}/${form.marca!.id_marca}`;
            try {
              const res = await authFetch(url, {
              ...
              show(crear ? 'Marca creada' : 'Marca modificada', 'success');
              setForm(null);
              load();
            } catch (e: any) {
      ...
      {del && (
        <ConfirmDelete
          ...
          onClose={() => setDel(null)}
          onConfirm={async () => {
            const url = `${ELIMINAR_URL}/${del.id_marca}`;
            try {
      ...
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="artm-head">
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="artm-title">{marca.descripcion.trim()}</div>
          </div>
        ...
        <div className="artm-grid">
          <div className="artm-row">
            ...
            <span className="artm-val">{marca.id_marca}</span>
          </div>
          <div className="artm-row">
            <span className="artm-ico"><ion-icon name="ribbon-outline" /></span>
            <span className="artm-label">Descripción</span>
            <span className="artm-val">{marca.descripcion.trim()}</span>
          </div>
        </div>
...
function MarcaForm({
  ...
  useEscClose(onClose);
  const [desc, setDesc] = useState(marca?.descripcion ?? '');
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="artm-head">
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 18, fontWeight: 700 }}>{mode === 'crear' ? 'Nueva marca' : 'Modificar marca'}</div>
            {marca && <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>#{marca.id_marca}</div>}
          </div>
          <button onClick={onClose} aria-label="Cerrar">
            <ion-icon name="close" style={{ fontSize: 24, color: 'var(--text-muted)' }} />
          </button>
        ...
        <button className="cpw-submit" onClick={submit} disabled={!valid || saving}>
          {saving ? 'Guardando…' : mode === 'crear' ? 'Crear marca' : 'Guardar cambios'}
...
function ConfirmDelete({ marca, onClose, onConfirm }: { marca: Marca; onClose: () => void; onConfirm: () => Promise<void> }) {
  useEscClose(onClose);
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-sheet confirm-sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="confirm-title">Eliminar marca</div>
        <div className="confirm-text">
          ¿Seguro que querés eliminar <strong>{marca.descripcion.trim()}</strong> (#{marca.id_marca})? Esta acción no se puede deshacer.
        </div>
